...
class MessageDB {
  ...
  Future<int> insert(WKMsg msg) async {
    ...
    var map = getMap(msg);
    var db = WKDBHelper.shared.getDB();
    if (db == null) {
      // 数据库未初始化，尝试初始化
      Logs.warn('数据库未初始化，尝试重新初始化');
      bool initResult = await WKDBHelper.shared.init();
      if (!initResult) {
        throw Exception("Database initialization failed");
      }
      db = WKDBHelper.shared.getDB();
      if (db == null) {
        throw Exception("Database not initialized after retry");
      }
    }
    return await db.insert(WKDBConst.tableMessage, map,
  ...
  updateSendingMsgFail() async {
    try {
      // 获取数据库实例并保存到局部变量，避免多次调用时空值
      final db = WKDBHelper.shared.getDB();
      if (db == null) {
        print('数据库未初始化，跳过更新发送中消息状态');
        return;
      }
      ...
      // 再次检查数据库是否还可用
      final dbForUpdate = WKDBHelper.shared.getDB();
      if (dbForUpdate == null) {
        print('数据库在更新前被关闭，跳过更新发送中消息状态');
        return;
      }

      int updatedRows = await dbForUpdate.update(WKDBConst.tableMessage, map,
          where: 'status=0');

      print('更新发送中消息状态完成，影响行数: $updatedRows');
    } catch (e) {
      print('更新发送中消息状态失败: $e');
      // 不抛出异常，避免影响初始化流程
    }
