...
//                oldestOrderSeq = 0;
  ...
  updateSendingMsgFail({bool isInitialization = false}) async {
    await MessageDB.shared.updateSendingMsgFail();

    // 如果不是初始化调用，且当前已连接，则启动重发管理器
    if (!isInitialization && !WKIM.shared.connectionManager.isDisconnection) {
      WKIM.shared.optimizedMessageResendManager.start();
      Logs.info('消息发送失败，启动重发管理器');
    } else if (isInitialization) {
      Logs.debug('初始化时更新发送中消息状态，不启动重发管理器');
    } else {
      Logs.debug('连接未建立，不启动重发管理器');
    }
  }

  /// 处理运行时消息发送失败
  /// 这个方法专门用于处理运行时的消息发送失败，会启动重发机制
  void handleMessageSendFailure(int clientSeq, String reason) {
    Logs.warn('消息发送失败: clientSeq=$clientSeq, reason=$reason');

    // 更新单条消息状态
    updateMsgStatusFail(clientSeq);

    // 如果当前已连接，启动重发管理器
    if (!WKIM.shared.connectionManager.isDisconnection) {
      WKIM.shared.optimizedMessageResendManager.start();
      Logs.info('消息发送失败，启动重发管理器进行重试');
    } else {
      Logs.debug('连接未建立，消息将在连接恢复后重发');
    }
  }
  ...
  /// 同步频道消息（供MessageSyncManager调用）
  Future<void> syncChannelMessages(
      String channelId, int channelType, int startSeq, int endSeq) async {
    // 这里是同步消息的具体实现
    // 调用现有的同步逻辑
    setSyncChannelMsgListener(
        channelId, channelType, startSeq, endSeq, endSeq - startSeq + 1, 0,
        (result) {
      // 同步完成的回调处理
      if (result != null) {
        Logs.message(
            '频道消息同步完成: $channelId:$channelType, 消息数: ${result.messages?.length ?? 0}');
      }
    });
  }
  ...
  List<Map<String, dynamic>> getAllSendingMessages() {
    return WKIM.shared.optimizedMessageResendManager.getAllSendingMessages();
  }

  /// 获取重发统计信息
  Map<String, dynamic> getResendStatistics() {
    return WKIM.shared.optimizedMessageResendManager.getStatistics();
  }

  /// 手动触发消息重发
  Future<void> manualResendMessages() async {
    await WKIM.shared.optimizedMessageResendManager.resendAllMessages();
  }
}
