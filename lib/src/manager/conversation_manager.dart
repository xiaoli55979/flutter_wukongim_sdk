...
class WKConversationManager {
  ...
  static final WKConversationManager _instance = WKConversationManager._privateConstructor();
  ...
  /// 同步会话回调
  Function(String lastSsgSeqs, int msgCount, int version, Function(WKSyncConversation))? _syncConversationBack;
  ...
  Future<WKUIConversationMsg?> saveWithLiMMsg(WKMsg msg, int redDot) async {
    ...
    if (msg.channelType == WKChannelType.communityTopic && msg.channelID.isNotEmpty) {
    ...
    WKUIConversationMsg? uiMsg = await ConversationDB.shared.insertOrUpdateWithConvMsg(wkConversationMsg);
    return uiMsg;
  }
  ...
  /// 获取指定频道的会话
  Future<WKUIConversationMsg?> getWithChannel(String channelID, int channelType) async {
    var msg = await ConversationDB.shared.queryMsgByMsgChannelId(channelID, channelType);
    if (msg != null) {
  ...
  Future<void> updateRedDot(String channelID, int channelType, int redDot) async {
    ...
    var result = await ConversationDB.shared.updateWithField(map, channelID, channelType);
    if (result > 0) {
  ...
  void addOnRefreshMsgListener(String key, Function(WKUIConversationMsg, bool) listener) {
  ...
  /// 添加会话列表刷新监听器
  void addOnRefreshMsgListListener(String key, Function(List<WKUIConversationMsg>) listener) {
  ...
  /// 添加同步会话监听器
  void addOnSyncConversationListener(Function(String lastSsgSeqs, int msgCount, int version, Function(WKSyncConversation)) listener) {
  ...
  void _saveSyncConversation(WKSyncConversation? syncChat) {
    if (syncChat == null || syncChat.conversations == null || syncChat.conversations!.isEmpty) {
      return;
    }
    // 初始化数据集合
    ...
    if (syncChat.conversations != null && syncChat.conversations!.isNotEmpty) {
      for (int i = 0, size = syncChat.conversations!.length; i < size; i++) {
        ...
        conversationMsg.lastClientMsgNO = syncChat.conversations![i].lastClientMsgNO;
        ...
        WKUIConversationMsg uiMsg = ConversationDB.shared.getUIMsg(conversationMsg);

        // 处理最近消息
        if (syncChat.conversations![i].recents != null && syncChat.conversations![i].recents!.isNotEmpty) {
          for (WKSyncMsg wkSyncRecent in syncChat.conversations![i].recents!) {
            ...
            if (wkSyncRecent.messageExtra != null) {
              WKMsgExtra extra =
                  WKIM.shared.messageManager.wkSyncExtraMsg2WKMsgExtra(msg.channelID, msg.channelType, wkSyncRecent.messageExtra!);
